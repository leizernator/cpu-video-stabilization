...
- **Translation Limit:** The `translation_limit` parameter (default `None`) sets the maximum stabilization correction the algorithm is permitted to apply in the X and Y axes, defined as a percentage of the frame dimensions. For example, `translation_limit=0.15` restricts corrections to 15% of the frame's width and height, preserving panning intention and eliminating off-screen border clipping.
- **6 DOF Movement Tracking:** The algorithm internally computes the 6 Degrees of Freedom (translation and rotation) camera movement. You can provide an optional `fov` (diagonal field of view in degrees) or `focal_length` (in pixels) for more accurate intrinsic camera approximation. Use `stabilizer.get_movement()` to retrieve the movement proportions of the most recently processed frame.
...
while True:
    ...
    # Retrieve the 6 DOF camera movement
    movement = stabilizer.get_movement()
